...
export class Pessoas extends Component {
    constructor(props) {
        ...
        this.exibirMensagemCadastro = this.exibirMensagemCadastro.bind(this);
    ...
    inserirPessoa(pessoa) {
        fetch('http://localhost:3003/pessoas', {
            ...
            if (resposta.ok) {
                ...
                this.exibirMensagemCadastro(data.data_nascimento);
                this.buscarPessoas();
                this.fecharModal();
            } else {
                this.fecharModal();
    ...
    exibirMensagemCadastro(dataNascimento) {
        ...
        if (mesAtual < mesNascimento || (mesAtual === mesNascimento && hoje.getDate() < dataNascimentoDate.getDate())) {
            idade--;
        }
    
        // Calcula os dias faltantes para o próximo aniversário
        const proximoAniversario = new Date(hoje.getFullYear(), dataNascimentoDate.getMonth(), dataNascimentoDate.getDate());
        if (hoje > proximoAniversario) {
            proximoAniversario.setFullYear(proximoAniversario.getFullYear() + 1);
        }
        const diasFaltantes = Math.ceil((proximoAniversario - hoje) / (1000 * 60 * 60 * 24));
    
        // Exibe a mensagem de sucesso personalizada
        let mensagem = `Cadastro bem-sucedido! Idade: ${idade} anos.`;
        if (hoje.getMonth() === dataNascimentoDate.getMonth() && hoje.getDate() === dataNascimentoDate.getDate()) {
            mensagem += ' Feliz aniversário!';
        } else {
            mensagem += ` Faltam ${diasFaltantes} dias para o próximo aniversário.`;
        }
        this.setState({ successMessage: mensagem });
    }
